/*
This program implement 2D convolution model using SPMD model using hybrid programming MPIopenMP.

This program is compiled by following commands
$ mpicc -c MPIopenMP.c -fopenmp
   $ mpicc -o MPIopenMP MPIopenMP.o -fopenmp
   
   default inputs are sample/1_im1 and sample/1_im2
   
   out_matrix_MPIopenMP is created in working directory
*/
...
/* Scatter C to the other processes */
  ...
  /* Inverse fast fourier transform on columns of C*/
   ...
   /* Write output file */
   write_matrix("output_matrix_MPI_OpenMP", C);
